...
function FeedItem({ userInfo, feedInfo, channelLabels, userLabels }) {
  ...
  if (channelId && channelId !== DefChannels.post) {
  ...
  return <div className="feed-item-cont">
    <div className="feed-item-image">{(headerSplit[0] || " ")[0].toLocaleUpperCase()+(headerSplit[1] || " ")[0].toLocaleUpperCase()}</div>
    <div className="feed-item-text-section">
      ...
      <div title={liked ? "Unlike" : "Like"} className={`feed-like-button ${liked ? "liked" : ""}`} onClick={() => {
        handleLikeClicked()
      }} />
    </div>
  </div>;
